...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('🔄 Iniciando upload de imagem de link para MinIO...')
    ...
    let fileName = `${session.user.id}_link_${Date.now()}.webp`
    
    try {
      // Redimensionar para 800x600 (fit inside)
      ...
      fileName = `${session.user.id}_link_${Date.now()}.jpg`
    }
    ...
    console.error('❌ Erro no upload da imagem de link:', error)
